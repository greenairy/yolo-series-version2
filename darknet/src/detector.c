...
float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, network *existing_net)
{
    ...
    //const float thresh_calc_avg_iou = 0.5;
    float avg_iou = 0;
    int tp_for_thresh = 0;
    int fp_for_thresh = 0;
...
void calc_anchors(char *datacfg, int num_of_clusters, int width, int height, int show)
{
    ...
    if (show) {
      ...
      {
         for(int w = 0 ; w<img->width*3 ; w++)
         {
            ((uchar*)((int)img->imageData + img->widthStep*h))[w] = (uchar) 255;  
        ...
        for (j = 0; j < num_of_clusters; ++j) {
            ...
            cvRectangle(img, pt1, pt2, CV_RGB(0,0, 0), 2, 8, 0);
        }
        ...
        for (i = 0; i < number_of_boxes; ++i) {
            ...
            cvCircle(img, pt, 2, CV_RGB(red_id, green_id, blue_id), CV_FILLED, 8, 0);
            //if(pt.x > img_size || pt.y > img_size) printf("\n pt.x = %d, pt.y = %d \n", pt.x, pt.y);
        }
        cvShowImage("clusters", img);
        cvSaveImage("clusters.jpg", img, 0);
        cvWaitKey(0);
        cvReleaseImage(&img);
        cvDestroyAllWindows();
        cvReleaseMat(&labels);
